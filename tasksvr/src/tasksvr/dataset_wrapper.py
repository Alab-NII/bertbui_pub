...
class DatasetWrapper(object):
    ...
    @staticmethod
    def _extract_zip(local_dir, zip_file_path):
...
class Wrappers(object):
    ...
    @staticmethod
    def get_default_wrappers_dir():

        return os.path.join(os.path.dirname(__file__), 'wrappers')

    @staticmethod
    def enumerate(data_dir, wrappers_path=None):
        
        ignored_files = ['__init__.py']
        wrappers_path = wrappers_path or Wrappers.get_default_wrappers_dir()
        wrappers = {}
        
        for filename in os.listdir(wrappers_path):
            
            if filename.endswith('.py') and filename not in ignored_files:
                
                filepath = os.path.join(wrappers_path, filename)
                module_name = 'wrapper_%s' % filename.replace('.py', '')
                ...
                assert hasattr(module, 'Wrapper'), f'Wrapper not found in {filepath}'
                wrapper_name = module.Wrapper.name

                assert wrapper_name not in wrappers, 'module already exists: %s' % wrapper_name
                wrappers[wrapper_name] = module.Wrapper(data_dir)
